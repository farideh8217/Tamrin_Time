...
if(isset($_POST["submit1"],$_POST["values"])) {
    $values = $_POST["values"];
    foreach ($values as $activity_id => $value) {
        if(isset($_POST["normal"],$_POST["extra"]))
        $normal = $value["normal"];
        $extra = $value["extra"];
...
<form action="reports.php" method="POST">
    ...
    <?php foreach($activities as $activity_item) {  ?>
        <div style="width: 200px ; height: 200px; background-color: coral">
            ...
            ساعت کار عادی<input type="text" name="values[<?= $activity_item["id"] ?>][normal]"><br>
            ساعت کار اضافه<input type="text" name="values[<?= $activity_item["id"] ?>][extra]"><br>
        </div>
    <?php } ?>
